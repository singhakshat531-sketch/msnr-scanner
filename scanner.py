...
def main():
    ...
    if bias != "RANGING":
        for lv in lvl_4h:
            ...
            if not is_near_level(cur_price, lv["price"]):
                continue
            ...
            # Find 1D target in same direction
            lv_1d_target = next((
                l for l in sorted(lvl_1d, key=lambda x: abs(x["price"]-lv["price"]))
                if l["type"] == lv["type"]
                and ((bull and l["price"] > lv["price"])
            ...
            mark_alerted(state, key)
            alerts_sent += 1
            print(f"\n  ✓ ALERT SENT: {lv['type']}-level ${lv['price']:,.0f} "
                  f"{grade.upper()} — price is {lv['dist']:.2f}% away")

    if alerts_sent == 0:
        print("\nNo levels being approached — no alert sent")
